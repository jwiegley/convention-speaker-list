    ...
    <div className="h-screen bg-gray-900 text-white p-2 flex flex-col overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-2 mb-2 flex-shrink-0" style={{ height: '32vh' }}>
        ...
        <div className="bg-gradient-to-br from-green-600 to-green-800 rounded-xl p-4 flex flex-col shadow-2xl">
          <h2 className="text-xl font-bold mb-2 text-green-100 flex-shrink-0">
            Currently Speaking
          </h2>
          <div className="flex-grow flex items-center justify-center">
            {currentSpeaker ? (
              <div
                className={`rounded-lg p-4 w-full h-full flex flex-col justify-center ${
                  currentSpeaker.delegate.has_spoken
                    ? 'bg-blue-600 text-white'
                    : 'bg-yellow-400 text-black'
                }`}
              >
                <div
                  className="text-9xl font-bold text-center mb-2"
                  style={{ fontSize: '10rem', lineHeight: '1' }}
                >
                  {currentSpeaker.delegate.number}
                </div>
                <div className="text-5xl text-center font-semibold">
                  {currentSpeaker.delegate.name}
                </div>
              </div>
            ) : (
        ...
        <div className="bg-gradient-to-br from-blue-600 to-blue-800 rounded-xl p-4 flex flex-col shadow-2xl">
          ...
          <div
            className={`flex-grow flex flex-col gap-2 ${nextTwo.length === 1 ? 'justify-start' : 'justify-center'}`}
          >
            {nextTwo.length > 0 ? (
      ...
      <div className="grid grid-cols-2 gap-2 flex-grow overflow-hidden">
        ...
        <div className="bg-gray-800 rounded-xl p-3 shadow-xl flex flex-col overflow-hidden">
          <h3 className="text-lg font-bold mb-2 text-gray-200 flex-shrink-0">Next 10 Speakers</h3>
            ...
            {nextTen.length > 0 ? (
              nextTen.map((item, index) => (
                ...
                >
                  ...
                  <div
                    className={`text-sm font-semibold ${item.delegate.has_spoken ? 'text-gray-200' : 'text-gray-700'}`}
                  >
                    {index + 3}
                  </div>
                </div>
