...
export class TimerService {
  ...
  startTimer(sessionId: string, duration: number, delegateId?: string): void {
    ...
    logger.info(
      `Timer started for session ${sessionId}${delegateId ? ` (delegate: ${delegateId})` : ''}, duration: ${state.duration}s`
    );
  }
  ...
  private handleTimerTick(timerKey: string): void {
    ...
    if (state.remainingTime === 0) {
      ...
      this.stopTimerByKey(timerKey);
      logger.info(
        `Timer expired for session ${state.sessionId}${state.delegateId ? ` (delegate: ${state.delegateId})` : ''}`
      );
    }
  }
  ...
  pauseTimer(sessionId: string, delegateId?: string): void {
    ...
    logger.info(
      `Timer paused for session ${state.sessionId}${state.delegateId ? ` (delegate: ${state.delegateId})` : ''}, remaining: ${state.remainingTime}s`
    );
  }
  ...
  resumeTimer(sessionId: string, delegateId?: string): void {
    ...
    logger.info(
      `Timer resumed for session ${state.sessionId}${state.delegateId ? ` (delegate: ${state.delegateId})` : ''}, remaining: ${state.remainingTime}s`
    );
  }
  ...
  private stopTimerByKey(timerKey: string): void {
    ...
    if (state) {
      ...
      logger.info(
        `Timer stopped for session ${state.sessionId}${state.delegateId ? ` (delegate: ${state.delegateId})` : ''}`
      );
    }
  }
  ...
  resetTimer(sessionId: string, delegateId?: string): void {
    ...
    logger.info(
      `Timer reset for session ${sessionId}${delegateId ? ` (delegate: ${delegateId})` : ''}`
    );
  }
  ...
  private emitTimerEvent(
    ...
    logger.debug(
      `Timer event: ${type} for session ${sessionId}${delegateId ? ` (delegate: ${delegateId})` : ''}, remaining: ${remainingTime}s`
    );
  }
