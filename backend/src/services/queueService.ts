...
import { IQueueService, IQueueAdvanceResult, IQueuePosition, IQueueState } from './interfaces/IQueueService';
import { QueueEntry, QueueOperationOptions, QueueEvent } from '../types/queue';
import { 
  QueueError,
  DuplicateEntryError,
...
export class QueueService implements IQueueService {
  ...
  async validateDelegate(delegateId: string): Promise<boolean> {
    ...
    try {
      const result = await client.query(
        'SELECT id, is_active FROM delegates WHERE id = $1',
        [delegateId]
      );
   ...
   */
  async addToQueue(delegateId: string, sessionId: string, options: QueueOperationOptions = {}): Promise<IQueueItem> {
    ...
    try {
      ...
      // Calculate position
      const position = options.forcePriority ?? await this.calculateQueuePosition(delegateId, sessionId);
   ...
   */
  private async findNextAvailablePosition(sessionId: string, startPosition: number): Promise<number> {
  ...
  async advanceQueue(sessionId: string): Promise<IQueueAdvanceResult> {
    ...
    try {
      ...
      const result: IQueueAdvanceResult = {
        previousSpeaker: currentSpeaker.rows[0] ? this.mapToQueueItem(currentSpeaker.rows[0]) : null,
