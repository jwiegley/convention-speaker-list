...
const firstNames = {
  ...
  Other: ['Alex', 'Jordan', 'Taylor', 'Morgan', 'Casey', 'Riley', 'Jamie', 'Avery', 'Quinn', 'Sage']
};

const lastNames = ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis', 'Rodriguez', 'Martinez',
  'Hernandez', 'Lopez', 'Gonzalez', 'Wilson', 'Anderson', 'Thomas', 'Taylor', 'Moore', 'Jackson', 'Martin'];

const locations = ['California', 'Texas', 'Florida', 'New York', 'Pennsylvania', 'Illinois', 'Ohio', 'Georgia',
  'North Carolina', 'Michigan', 'New Jersey', 'Virginia', 'Washington', 'Arizona', 'Massachusetts'];

function generateDelegates(count: number): SeedDelegate[] {
  const delegates: SeedDelegate[] = [];
  const genders: Array<'Male' | 'Female' | 'Other'> = ['Male', 'Female', 'Other'];
  const ageBrackets: Array<'20s' | '30s' | '40s' | '50s' | '60s' | '70s+'> = ['20s', '30s', '40s', '50s', '60s', '70s+'];
  const raceCategories: Array<'White_Persian' | 'Non_White_Non_Persian'> = ['White_Persian', 'Non_White_Non_Persian'];
  ...
  try {
    ...
    for (const delegate of delegates) {
      await pool.query(
        ...
        [delegate.id, delegate.delegate_number, delegate.name, delegate.location, 
         delegate.gender, delegate.age_bracket, delegate.race_category, delegate.has_spoken]
      );
    }
