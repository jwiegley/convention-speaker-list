  ...
  timerService.onTimerEvent((event: TimerEvent) => {
    ...
    switch (type) {
      case 'start':
        ...
        logger.info(`Timer started broadcast for session ${sessionId}${delegateId ? ` (delegate: ${delegateId})` : ''}`);
        break;
      ...
      case 'pause':
        ...
        logger.info(`Timer paused broadcast for session ${sessionId}${delegateId ? ` (delegate: ${delegateId})` : ''}`);
        break;
      ...
      case 'resume':
        ...
        logger.info(`Timer resumed broadcast for session ${sessionId}${delegateId ? ` (delegate: ${delegateId})` : ''}`);
        break;
      ...
      case 'stop':
        ...
        logger.info(`Timer stopped broadcast for session ${sessionId}${delegateId ? ` (delegate: ${delegateId})` : ''}`);
        break;
 ...
 */
export function setupTimerHandlers(socket: SocketClient, io: SocketServer): void {
  // Handle timer start
