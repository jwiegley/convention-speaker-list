      ...
      switch (type) {
          ...
          if (data && data.id) {
            ...
            if (queueItem) {
              ...
              if (
                ...
                !timerService.hasActiveTimer(sessionId, queueItem.delegateId)
              ) {
                timerService.startTimer(sessionId, 180, queueItem.delegateId); // 3 minutes
                logger.info(
                  `Auto-started timer for delegate ${queueItem.delegateId} added to speaking position in session ${sessionId}`
                );
        ...
        case 'advanced':
          ...
          if (data) {
            ...
            const nextSpeaker = queueState.items.find(
              (item) => item.position === 1 && item.status === 'waiting'
            );
            ...
            if (currentSpeaker) {
              ...
              if (!timerService.hasActiveTimer(sessionId, currentSpeaker.delegateId)) {
                ...
                logger.info(
                  `Auto-started timer for delegate ${currentSpeaker.delegateId} in session ${sessionId}`
                );
              ...
              if (data.previousSpeaker && data.previousSpeaker.delegateId) {
                if (timerService.hasActiveTimer(sessionId, data.previousSpeaker.delegateId)) {
                  ...
                  logger.info(
                    `Auto-stopped timer for previous speaker ${data.previousSpeaker.delegateId} in session ${sessionId}`
                  );
                }
          ...
          emitQueueUpdate(
            ...
            sessionId,
            {
              ...
              action: 'advanced',
              affectedItems: data
                ? [data.currentSpeakerId, data.previousSpeakerId].filter(Boolean)
                : [],
            },
            true
          ); // Immediate emit for speaker advancement
...
export async function sendQueueSnapshot(
  ...
  try {
    ...
    const onDeck = queueState.items.filter((item) => item.status === 'waiting').slice(0, 3);

    emitQueueSnapshot(io, socketId, sessionId, queueState.items, currentSpeaker, onDeck);
